...
def load_data(module_path,problem, data_filename):
    ...
    (data_features, data_targets) = (None, None)
    

    # Obtain the path to the data and the rankings
    # for Label Ranking and Partial Label Ranking
    if problem == "label_ranking":
        data_filename_lr = join(
            module_path,
            "data", "label_ranking", "ranks",
            data_filename)
        if not isfile(data_filename_lr):
            raise IOError("Could not find dataset" + data_filename_lr)
        dataframe = pd.read_csv(filepath_or_buffer=data_filename_lr,
                                delimiter=",",
                                header=0,
                                dtype=str)
    else:
        # problem must be partial_label_ranking
        data_filename_plr = join(
            module_path,
            "data", "partial_label_ranking", "ranks",
            data_filename)
        if not isfile(data_filename_plr):
            raise IOError("Could not find dataset" + data_filename_plr)
        
        dataframe = pd.read_csv(filepath_or_buffer=data_filename_plr,
                                delimiter=",",
                                header=0,
                                dtype=str)
    ...
    (colname_features, colname_classes) = (
        list(filter(lambda x: x.startswith("A"), dataframe.columns)),
        list(filter(lambda x: x.startswith("L"), dataframe.columns)))

    # Finally, read the .csv files
    # with the data and the rankings
    (data_features, data_targets) = (
            dataframe[colname_features],
            dataframe[colname_classes].astype(np.int64))
    # Return the data and the rankings
    return (data_features, data_targets)


def load_authorship(problem):
    ...
    """
    if problem not in [ "label_ranking", "partial_label_ranking"]:
        raise TypeError("problem must be one of the following values: label_ranking or partial_label_ranking")
    ...
    #Update Column names respective to above declarations
    data_features = data_features.set_axis(feature_names, axis=1)
    data_targets = data_targets.set_axis(class_names, axis=1) 
